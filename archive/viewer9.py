...
from PySide6.QtWidgets import (
    ...
    QSplitter, QDialog, QTextEdit, QDialogButtonBox,
    QTabWidget, QFormLayout, QCheckBox
)
from PySide6.QtGui import (
    QPixmap, QPalette, QColor, QAction, QFont, QIcon, QKeySequence, QScreen,
    QPainter, QBrush, QPen
)
from PySide6.QtCore import (
    Qt, QPoint, QDir, QModelIndex, QRegularExpression,
    QSortFilterProxyModel, Signal, QEvent, QTimer, QPropertyAnimation, QEasingCurve, Property, QRect
)
                ...
                if old_path in self.metadata_cache:
                    ...
                    if self.parent():
                        ...
                        try:
                            ...
                            cur = con.cursor()
                            cur.execute("DELETE FROM song_metadata WHERE file_path = ?", (old_path,))
                            con.commit()
                            con.close()
...
class PraiseSheetViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("물댄동산 악보 뷰어 Pet1 2:9 V2.4")
        ...
        self.proxy_model = CustomSortFilterProxyModel(self.all_extensions, self.favorites, self.metadata_cache, self)
        ...
        self.google_search_input.setPlaceholderText("Google에서 악보 검색...")
        self.btn_google_sheet_search = QPushButton("검색")
        self.btn_google_sheet_search.setFixedWidth(80)
        self.btn_google_sheet_search.clicked.connect(self.search_sheet_music_on_google)
        self.google_search_input.returnPressed.connect(self.search_sheet_music_on_google) 
        google_search_layout = QHBoxLayout()
        google_search_layout.addWidget(QLabel("Google 악보:"))
        google_search_layout.addWidget(self.google_search_input)
        google_search_layout.addWidget(self.btn_google_sheet_search)

        self.sheet_sort_combo = QComboBox()
        self.sheet_sort_combo.addItems(["이름순 (오름차순)", "이름순 (내림차순)", "키(Key)순 (오름차순)", "키(Key)순 (내림차순)"])
        self.sheet_sort_combo.currentTextChanged.connect(self.change_sheet_sort_order)

        self.key_filter_combo = QComboBox()
        self.key_filter_combo.addItems(["전체", "A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "미지정"])
        ...
        self.playlist_sort_combo.addItems(["수정날짜순 (최신)", "수정날짜순 (오래된)", "이름순 (오름차순)", "이름순 (내림차순)"])
        ...
        self.inspector_key_combo.addItems(["", "A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#"])

        self.inspector_lyrics_edit = QTextEdit()
        self.inspector_lyrics_edit.setAcceptRichText(False)
        self.inspector_lyrics_edit.setPlaceholderText("가사 전체 또는 검색에 사용할 핵심 구절을 입력하세요.")
        ...
        self.inspector_key_combo.currentTextChanged.connect(self.on_inspector_key_changed)
        ...
        top_layout.addLayout(google_search_layout)
        ...
        dual_control_layout.addLayout(screen_control_layout) # 화면 제어 버튼 추가
        ...
        # [중요] 리스트 위젯에 Stretch를 줘서 남는 공간을 다 차지하게 함 (여백 제거 효과)
        ...
        right_layout.addLayout(logo_layout) # 로고 설정 추가
    ...
    def eventFilter(self, obj, event):
        ...
        if (hasattr(self, 'viewer') and self.viewer and
                obj == self.viewer.scroll_area.viewport() and event.type() == QEvent.Wheel):
    ...
    def get_metadata_from_db(self, file_path):
        try:
            con = sqlite3.connect(self.db_path)
            cur = con.cursor()
            cur.execute("SELECT song_key, lyrics FROM song_metadata WHERE file_path = ?", (file_path,))
            result = cur.fetchone()
            con.close()
            if result:
                return result
    ...
    def load_metadata_to_inspector(self, path):
        if path is None or not os.path.isfile(path) or not path.lower().endswith(tuple(self.image_extensions)):
            self.inspector_current_path = None
    ...
    def search_sheet_music_on_google(self):
        search_text = self.google_search_input.text().strip()
        if not search_text:
            QMessageBox.warning(self, "알림", "검색어를 입력하세요.")
            return
        if "악보" not in search_text:
            query = f"{search_text} 악보"
        else:
            query = search_text
        try:
            safe_query = urllib.parse.quote_plus(query)
        ...
        if item is not None and item != self.current_playlist_tooltip_item:
            ...
            if path and os.path.isfile(path) and path.lower().endswith(tuple(self.image_extensions)):
                ...
                if not pixmap.isNull():
                    ...
                    QToolTip.showText(event.globalPosition().toPoint() + QPoint(20, 20), tooltip, self.preview_list_widget)
                else:
                    QToolTip.hideText()
            else:
                QToolTip.hideText()
    ...
    def start_single_song_show(self):
        ...
        # 뷰어 열기 (또는 갱신)
        self.open_viewer_window(paths, 0)
    ...
    def get_themes(self):
        return {
            "그린": {"base": "#F0FFF0", "window": "#E6F5E6", "text": "#003300", "button": "#90EE90", "button_text": "#003300", "highlight": "#3CB371", "highlight_text": "#FFFFFF", "border": "#2E8B57",},
            "기본 (밝게)": {"base": "#FFFFFF", "window": "#FFFFFF", "text": "#000000", "button": "#F5F5F5", "button_text": "#000000", "highlight": "#D3D3D3", "highlight_text": "#000000", "border": "#CCCCCC",},
            "어둡게": {"base": "#3E3E3E", "window": "#2D2D2D", "text": "#E0E0E0", "button": "#555555", "button_text": "#E0E0E0", "highlight": "#BB86FC", "highlight_text": "#000000", "border": "#1E1E1E",},
            "클래식 블루": {"base": "#EAF2F8", "window": "#D4E6F1", "text": "#1A5276", "button": "#A9CCE3", "button_text": "#154360", "highlight": "#5DADE2", "highlight_text": "#FFFFFF", "border": "#A9CCE3",}
        }
        ...
        try:
            if os.path.exists(self.settings_file):
                with open(self.settings_file, 'r', encoding='utf-8') as f:
                    ...
                    self.current_theme = settings.get('current_theme', self.current_theme)
    ...
    def change_logo_image(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "로고 이미지 선택", self.logo_image_path, "Images (*.png *.jpg *.jpeg *.bmp *.gif)")
        if file_path:
            self.logo_image_path = file_path
            self.logo_path_label.setText(os.path.normpath(self.logo_image_path))
            self.save_settings()
            # 뷰어가 열려있다면 즉시 업데이트도 가능하겠지만, 단순화를 위해 재시작 시 반영하도록 하거나
            # 여기서는 설정만 저장하고 다음에 뷰어 열 때 반영됩니다.
    ...
    def apply_search_filter(self, text):
        ...
        self.proxy_model.setFilterRegularExpression(QRegularExpression(pattern, QRegularExpression.CaseInsensitiveOption))
    ...
    def apply_playlist_search_filter(self, text):
        ...
        if not keywords:
            ...
            pattern = "".join([f"(?=.*{re.escape(keyword)})" for keyword in keywords])
        self.playlist_proxy_model.setFilterRegularExpression(QRegularExpression(pattern, QRegularExpression.CaseInsensitiveOption))
        self.playlist_tree.setRootIndex(self.playlist_proxy_model.mapFromSource(self.playlist_model.index(self.playlist_model.rootPath())))
    ...
    def delete_playlist_file(self, path):
        reply = QMessageBox.question(self, '파일 삭제 확인', f"'{os.path.basename(path)}' 파일을 정말로 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
    ...
    def handle_list_click(self, item):
        """리스트 아이템을 단순 클릭했을 때 듀얼모니터 쇼창이 켜져있으면 즉시 변경 (항상 활성화)"""
        # [비활성화됨] 클릭 시 즉시 쇼 기능
        pass
    ...
    def delete_all_items(self):
        reply = QMessageBox.question(self, '전체 삭제 확인', '정말로 모든 찬양 목록을 삭제하시겠습니까?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.list_widget.clear()
    ...
    def save_list(self):
        path, _ = QFileDialog.getSaveFileName(self, "리스트 저장", self.playlist_path, "Praise List Files (*.pls)")
        if path:
            if not path.lower().endswith(".pls"):
                path += ".pls"
            items_to_save = [os.path.relpath(self.list_widget.item(i).data(Qt.UserRole), self.sheet_music_path) for i in range(self.list_widget.count())]
            try:
    ...
    def load_list(self):
        path, _ = QFileDialog.getOpenFileName(self, "리스트 불러오기", self.playlist_path, "Praise List Files (*.pls);;All Files (*)")
        if path:
            self.list_widget.clear()
            self._add_paths_from_pls(path)
    ...
    def preview_selected_file(self, current_index, tree_view):
        ...
        is_song_file = (tree_view == self.tree) and (path and os.path.isfile(path)) and path.lower().endswith(tuple(self.image_extensions))
        if is_song_file:
    ...
    # --- [듀얼 모니터] 관련 메서드들 ---
    def open_viewer_window(self, paths, start_index):
        """쇼창(Viewer)을 열거나 업데이트합니다."""
        if not paths:
        ...
        if self.viewer and self.viewer.isVisible():
            self.viewer.update_content(paths, start_index, self.initial_zoom_percentage)
            self.viewer.activateWindow() # 창을 앞으로 가져옴
            return

        # 뷰어가 닫혀있으면 새로 생성 (로고 경로 전달 추가)
    ...
    def start_show_from_current(self):
        ...
        if start_index < 0:
            start_index = 0
        self.open_viewer_window(paths, start_index)
...
class FullScreenViewer(QWidget):
    ...
    def __init__(self, image_paths, initial_zoom_percentage=80, start_index=0, scroll_sensitivity=30, logo_path=""):
        super().__init__()
        ...
        if self.logo_path and os.path.isfile(self.logo_path):
            ...
            if not logo_pix.isNull():
                 # 로고는 화면에 꽉 차게 할지, 작게 할지 정책이 필요하지만, 여기선 적당히 크게
                self.logo_pixmap_original = logo_pix
            else:
                self.logo_screen_widget.setText("로고 이미지를 불러올 수 없습니다.")
                self.logo_screen_widget.setStyleSheet("color: white; background-color: black;")
        else:
    ...
    def update_content(self, image_paths, start_index, initial_zoom_percentage):
        """열려있는 상태에서 내용을 갱신합니다."""
        self.image_paths = image_paths
        self.current_index = start_index
        if not (0 <= self.current_index < len(self.image_paths)):
    ...
    def resizeEvent(self, event):
        ...
        if self.main_layout.currentWidget() == self.scroll_area:
            self.load_image()
        # 로고 모드일 때 로고 리사이즈
        if self.main_layout.currentWidget() == self.logo_screen_widget and hasattr(self, 'logo_pixmap_original'):
             self.display_logo_scaled()

    def display_logo_scaled(self):
        if hasattr(self, 'logo_pixmap_original'):
            scaled = self.logo_pixmap_original.scaled(self.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
            self.logo_screen_widget.setPixmap(scaled)

    def show_first_alert(self):
        # 쇼 화면에는 팝업을 띄우지 않는 것이 좋음 (비프음이나 무시)
        pass
    ...
    def toggle_black_screen(self):
        if self.main_layout.currentWidget() == self.black_screen_widget:
            self.main_layout.setCurrentWidget(self.scroll_area)
            self.update_next_song_label() # 돌아왔을 때 라벨 상태 복구
        else:
    ...
    def update_next_song_label(self):
        # 블랙 스크린이나 로고 스크린, 엔드 스크린일 때는 넥스트 라벨 숨김
        if self.main_layout.currentWidget() != self.scroll_area:
            self.next_song_label.hide()
            return
    ...
    def keyPressEvent(self, event):
        # [수정] ESC: 특수 화면이면 원래대로, 아니면 종료
        if event.key() == Qt.Key_Escape:
            if self.main_layout.currentWidget() in [self.black_screen_widget, self.logo_screen_widget]:
                self.main_layout.setCurrentWidget(self.scroll_area)
                self.update_next_song_label()
            else:
                self.close()
        ...
        # 페이지 넘김 로직 (블랙 스크린 상태에서도 내부적으로는 넘김)
        if event.key() in (Qt.Key_PageDown, Qt.Key_Right):
            if self.current_index < len(self.image_paths) - 1:
                self.current_index += 1
                self.load_image() # 내부 이미지 업데이트
                self.scroll_area.verticalScrollBar().setValue(0)
                self.scroll_area.horizontalScrollBar().setValue(0)
            else:
        ...
        # 스크롤 로직 (화면이 보일 때만 유효하게 하거나, 블랙일 때도 스크롤 되게 할 수 있음. 여기선 유지)
    ...
    def mousePressEvent(self, event):
        # 블랙/로고 스크린 상태에서도 마우스 클릭으로 넘길 것인가?
        # 보통 PPT 리모컨이 'PageDown'을 보내지만 마우스 클릭도 사용하므로
        # 여기서는 화면이 가려져 있어도 클릭하면 다음 곡으로 넘어가도록 유지합니다.
