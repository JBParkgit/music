...
class ToggleSwitch(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._checked = False
        self._bg_color_off = QColor("#777777")  # 끄기 색상 (회색)
        self._bg_color_on = QColor("#4CD964")   # 켜기 색상 (밝은 초록)
        self._circle_color = QColor("#FFFFFF")  # 원 색상 (흰색)
        
        # 애니메이션을 위한 변수
        self._circle_position = 3
        self._anim = QPropertyAnimation(self, b"circle_position", self)
        self._anim.setEasingCurve(QEasingCurve.InOutCubic)
        self._anim.setDuration(200)  # 0.2초 애니메이션
    ...
    def setChecked(self, checked):
        ...
        # 위치 강제 설정 (애니메이션 없이)
        end_pos = self.width() - 23 if checked else 3
        self.set_circle_position(end_pos)
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            end_pos = self.width() - 23 if self._checked else 3
    ...
    def paintEvent(self, event):
        ...
        if self._checked:
            painter.setBrush(QBrush(self._bg_color_on))
            painter.setPen(Qt.NoPen)
        else:
            painter.setBrush(QBrush(self._bg_color_off))
            painter.setPen(Qt.NoPen)
...
class PraiseSheetViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("물댄동산 악보 뷰어 Pet1 2:9 V2.3 (듀얼모니터 지원)")
        ...
        # [복구됨] 리스트 클릭 시 듀얼모니터 즉시 송출 로직 연결
        ...
        # --- [복구됨] 송출 제어 기능 ---
        self.btn_toggle_dual_viewer = QPushButton("송출창 켜기")
        self.btn_toggle_dual_viewer.setCheckable(True)
        self.btn_toggle_dual_viewer.clicked.connect(self.toggle_dual_monitor_viewer)
        
        # [신규] 토글 스위치 위젯 생성 및 레이아웃 배치
        self.switch_instant_show = ToggleSwitch()
        
        switch_layout = QHBoxLayout()
        switch_layout.addWidget(QLabel("클릭 시 즉시 송출:"))
        switch_layout.addWidget(self.switch_instant_show)
        ...
        dual_control_layout.addLayout(switch_layout) # 스위치 레이아웃 추가
        
        dual_group = QGroupBox("송출 설정")
    ...
    def handle_list_click(self, item):
        """리스트 아이템을 단순 클릭했을 때 듀얼모니터 송출창이 켜져있고 '즉시 송출' 스위치가 켜져있으면 변경"""
        if self.viewer and self.viewer.isVisible() and self.switch_instant_show.isChecked():
    ...
    def open_viewer_window(self, paths, start_index):
        ...
        self.btn_toggle_dual_viewer.setText("송출창 끄기")
    ...
    def on_viewer_closed(self):
        ...
        self.btn_toggle_dual_viewer.setText("송출창 켜기")
        self.viewer = None
...
class FullScreenViewer(QWidget):
    ...
    def __init__(self, image_paths, initial_zoom_percentage=80, start_index=0, scroll_sensitivity=30):
        super().__init__()
    ...
    def load_image(self):
        ...
        pixmap = QPixmap(path)
        if pixmap.isNull():
            # QMessageBox는 송출화면에 뜨면 안될 수도 있으나 일단 유지
            # 실전에서는 에러 이미지를 보여주는게 나을 수 있음
    ...
    def show_first_alert(self):
        # 송출 화면에는 팝업을 띄우지 않는 것이 좋음 (비프음이나 무시)
        pass
